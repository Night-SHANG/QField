...
7. 查询结果中的“地图 / 详情 / 附件 / 导航”始终可用；编辑模式再开放“编辑 / 巡检 / 维修 / 删除”。
8. 地图上直接点击单个点位/管线时继续复用 QField 原生识别与详情表单。详情旁的“照片/附件”可直接浏览已有照片缩略图和其他附件；编辑模式可以新增拍照、视频、录音或文档。
9. 删除不再使用 QField 原生“删除要素”。误建且没有巡检/维修历史的点位/管线可以确认删除，并同步删除直属附件；已有巡检/维修历史或仍有关联点位的管线禁止直接删除，改为将状态设为“停用”。
10. “我的位置”标记可以隐藏而不停止 GNSS。位置圆盘只保留现场真正有用的动作；轨迹跟踪、光标锁定、地图锁定、书签等专业 GNSS 功能在供水版隐藏。
11. QField 原生 Processing（“处理要素 / 收藏夹 / 所有算法”）、图层树和 Map Theme 在现场版隐藏，不要求一线人员理解 GIS 专业工具。
12. “更多”集中放显示/隐藏我的位置、复制当前位置、备份/导出、打开其他供水数据等低频操作。
