...
6. “新增”只在编辑模式可用，里面仅保留“新增点位 / 新建管线”。新增点位继续复用 QField 原生相机，可连续拍多张；新建管线继续复用 QField 原生数字化。进入画线状态后会隐藏供水版底栏和定位浮钮，并持续显示“点地图加节点 → 至少两点 → ✓ 完成 → 填参数”的说明。
7. 查询结果中的“地图 / 详情 / 附件 / 导航”始终可用；编辑模式只增加与管网资料维护有关的“编辑 / 删除”。巡检、维修入口不再作为现场主流程。
8. 地图上直接点击单个点位/管线时继续复用 QField 原生识别与详情表单。详情旁的“照片/附件”可直接浏览已有照片缩略图和其他附件；编辑模式可以新增拍照、视频、录音或文档。
9. 点位表单只把“设施类型 *”作为需要人工确认的必填项；坐标在放点时自动取得。名称、编号、片区、位置描述、照片、备注均可选。管线在完成画线后即可保存，管径、材质、主管/支管、压力分区等参数不知道时均可留空。“所属管线/连接关系”不要求人工填写，并从普通点位表单隐藏。
10. 删除不再使用 QField 原生“删除要素”。误建且没有巡检/维修历史的点位/管线可以确认删除，并同步删除直属附件；已有历史记录或仍有关联点位的管线禁止直接删除，改为将状态设为“停用”。
11. “我的位置”标记可以隐藏而不停止 GNSS。位置圆盘只保留现场真正有用的动作；轨迹跟踪、光标锁定、地图锁定、书签等专业 GNSS 功能在供水版隐藏。
12. QField 原生 Processing（“处理要素 / 收藏夹 / 所有算法”）、图层树和 Map Theme 在现场版隐藏，不要求一线人员理解 GIS 专业工具。
13. “更多”集中放显示/隐藏我的位置、复制当前位置、备份/导出、打开其他供水数据等低频操作。
