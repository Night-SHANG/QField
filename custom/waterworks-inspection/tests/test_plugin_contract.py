...
class PluginContractTests(unittest.TestCase):
    ...
    def test_basemap_switcher_keeps_business_data_separate(self) -> None:
        ...
        self.assertIn("t0.tianditu.gov.cn/", self.text)
        self.assertIn("/wmts?SERVICE=WMTS&REQUEST=GetTile", self.text)
        ...
        self.assertIn("function sanitizeTiandituProbeDetail(value)", self.text)
        self.assertIn('request.responseType = "text"', self.text)
        self.assertIn('request.getResponseHeader("Server")', self.text)
        self.assertIn("request.responseText", self.text)
        self.assertIn('detail.split(token).join("[密钥已隐藏]")', self.text)
